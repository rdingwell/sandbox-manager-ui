...
export default class Notification extends Component {
    ...
    render () {
        ...
        let labelStyles = { color: this.props.theme.primary3Color };
                ...
                <Menu className='notification-menu'>
                    ...
                    {pendingInvitations.map((p, key) =>
                        <MenuItem className='notification-menu-item' key={key}
                                  primaryText={<div>
                                      <div>
                                          <span className='label' style={labelStyles}>To: </span><span>{p.sandbox.name}</span>
                                      </div>
                                      <div>
                                          <span className='label' style={labelStyles}>From: </span><span>{p.invitedBy.name} / {p.invitedBy.email}</span>
                                      </div>
