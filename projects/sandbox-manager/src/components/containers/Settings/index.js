...
class Settings extends Component {
    ...
    render () {
        return <div className='settings-wrapper'>
            <Tabs className={(this.props.sandbox.visibility==='PRIVATE' ? 'settings-tabs' : 'settings-tabs-public-app')} contentContainerClassName='settings-tabs-container' inkBarStyle={{ backgroundColor: this.props.muiTheme.palette.primary2Color }}
                ...
                {this.props.sandbox.visibility==='PRIVATE' && <Tab label="Users" className={'sandbox-reset tab' + (this.state.activeTab === 'reset' ? ' active' : '')} onActive={() => this.setActiveTab('reset')}>
                    <UserManagement/>
                </Tab>}
            </Tabs>
        </div>;
